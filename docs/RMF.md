...
- The principle of ALARP (As Low As Reasonably Practicable) shall be applied to all hazards rated in the ALARP zone.
...
| 4 – 8 | ALARP | Risk reduction desirable; accept only if further reduction is not practicable or if benefits outweigh residual risk |
...
1. The residual RPN falls within the Acceptable (<4) or ALARP (4–8) zone; and
2. For ALARP residuals, it has been demonstrated that further risk reduction is not practicable without negating the clinical benefit of the device; and
...
The following matrix defines risk categories by the combination of Severity (S, 1–4) and Probability (P, 1–5). Cell values show RPN = S × P and the corresponding acceptability category (A = Acceptable, AL = ALARP, U = Unacceptable).

| | **P1** | **P2** | **P3** | **P4** | **P5** |
|---|---|---|---|---|---|
| **S4** | 4 AL | 8 AL | 12 U | 16 U | 20 U |
| **S3** | 3 A | 6 AL | 9 U | 12 U | 15 U |
| **S2** | 2 A | 4 AL | 6 AL | 8 AL | 10 U |
| **S1** | 1 A | 2 A | 3 A | 4 AL | 5 AL |
...
|---|---|---|---|
| H-001 | Incorrect biomarker score → erroneous clinical decision | 6 | ALARP |
...
| H-009 | Non-English L1 speaker scores misinterpreted | 4 | ALARP |
| H-010 | Noisy environment degrades acoustic scores | 4 | ALARP |

### 7.2 ALARP Justification

Three hazards remain in the ALARP zone after the application of risk control measures.

**H-001 (RPN = 6 after controls):** The residual probability cannot be reduced to P1 without removing the core functionality of the device. The fundamental limitation is that all speech biomarker systems carry inherent measurement uncertainty; this is not unique to CogAssess and is characteristic of the technology class. The risk is mitigated to ALARP by ensuring that (a) output is advisory only, (b) a qualified clinician is always in the decision loop, and (c) range validation prevents silent out-of-range scores from being displayed. Further reduction (e.g. requiring a second clinician sign-off on every assessment) would impose a disproportionate operational burden that would negate the clinical utility of the device.

**H-009 (RPN = 4 after controls):** The residual probability cannot be reduced to P1 without replacing the English-language AI models with multilingual equivalents, which is outside the scope of CogAssess v1.0. The ALARP residual is accepted on the basis that (a) L1 is a required intake field, (b) an automated amber warning is displayed on every report for non-English L1 speakers, and (c) clinician training explicitly addresses this limitation. The device is intended for English-language clinical settings; use in predominantly non-English-speaking populations is outside the current intended use.

**H-010 (RPN = 4 after controls):** The recording environment cannot be fully controlled by the software. The ALARP residual is accepted on the basis that (a) environment quality is captured at intake, (b) it is displayed as a confounding variable on the report, and (c) clinical guidance recommends quiet room assessment. Further reduction would require hardware integration (e.g. noise-cancelling microphones) outside the scope of the software device.

### 7.3 Conclusion

No identified hazard has a residual RPN in the Unacceptable zone (>8). All residual risks are either Acceptable (<4) or ALARP (4–8) with documented justification. The overall residual risk profile of CogAssess v1.0 is assessed as acceptable subject to the benefit-risk determination in Section 8.
...
The clinical benefit of CogAssess — earlier and more reproducible identification of speech biomarker changes associated with cognitive impairment — materially outweighs the residual risks identified in Section 6. All residual risks have been reduced to Acceptable or ALARP levels. The three ALARP hazards (H-001, H-009, H-010) are inherent to the technology class and are managed by design controls that keep a qualified clinician in the decision loop at all times.
...
| Hazards with ALARP initial RPN (4–8) | 6 (H-002, H-005, H-006, H-007, H-009, H-010) |
| Hazards with acceptable initial RPN (<4) | 2 (H-004, H-008) |
| Residual hazards in Unacceptable zone | 0 |
| Residual hazards in ALARP zone | 3 (H-001, H-009, H-010) |
| Residual hazards in Acceptable zone | 7 |
| Maximum residual RPN | 6 (H-001) |
| ALARP justification documented | Yes — Section 7.2 |
